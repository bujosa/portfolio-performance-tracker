...
export class AssetRepository {
  private readonly entityName = Asset.name;
  ...
  constructor(
    ...
    readonly entityModel: Model<Asset>,
  ) {}
  ...
  public async getAllEntities(filterInput: FilterInput): Promise<Asset[]> {
    try {
      let query = this.entityModel.find();

      const result: Asset[] = await mongooseQueryBuilder(query, filterInput);
  ...
  public async createEntity(
    createEntityInput: CreateAssetInput,
    session?: ClientSession,
  ): Promise<Asset> {
    try {
